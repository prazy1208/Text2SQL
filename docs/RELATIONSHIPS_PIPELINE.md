...
3. **Embeddings (optional)** — `python build_relationship_embeddings.py` writes `metadata_store/relationships_{schema}_metadata.json` (SentenceTransformer `all-MiniLM-L6-v2`). Runtime agents use the **database** full list, not FAISS, in v1.
4. **API** — `POST /query` resolves `use_case` → `schema_name`, loads rows via `backend/services/relationship_retrieval.py`, and passes them into the table and column agents.
...
| Runtime load | `backend/services/relationship_retrieval.py` |
...
- **Logging:** If `table_relationships` is missing or the query fails, `query.py` logs a warning and continues with an empty relationship list so the pipeline still responds.
